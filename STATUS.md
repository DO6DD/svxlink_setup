...
- Logrotate läuft für die Rotation als root: Das fehlerhafte `su svxlink svxlink` wurde entfernt, damit es in dem root-eigenen Verzeichnis `/var/log` Rotationsdateien anlegen kann. Die aktive `/var/log/svxlink` bleibt Eigentum von `svxlink:svxlink` und verwendet weiter `copytruncate`.
- Der ELENATA-Post-Boot-Helper lädt keine zweite Konfigurationsdatei mehr. Seine Mixerumgebung kommt ausschließlich aus dem `EnvironmentFile=` der Unit; damit tritt kein Abbruch durch die nicht gesetzte Variable `ELENATA_ALSA_CONFIG_FILE` mehr auf.
