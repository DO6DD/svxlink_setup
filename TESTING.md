...
- `tests/simulate_package_installation.sh` prüft die vollständige allgemeine Paketliste sowie die direkten Debian-Pakete `libsigc++-2.0-dev` und `libgcrypt20-dev`; die früheren Aliasnamen dürfen nicht angefordert werden. Debian 13 arm64 stellte beide konkreten Pakete bereit, während die frühere Alias-/Kandidatenprüfung sie fälschlich als nicht verfügbar meldete. Der reale Neuinstallationstest wurde deshalb dort abgebrochen und steht vollständig erneut an. `raspberrypi-kernel-headers` wird nicht mehr angefordert: ELENATA baut kein externes Kernelmodul.
